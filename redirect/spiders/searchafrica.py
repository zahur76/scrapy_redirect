...
mydb = myclient["searchafrica"]

companycol = mydb["company_links"]
...
class QuotesSpider(scrapy.Spider):
    ...
    def parse_two(self, response):
        # print(response.url)
        urls_ = []
        query = companycol.find({'complete': 0}).limit(10000)
        ...
        for q in urls_:
            ...
            yield scrapy.Request(url=q['company_url'], callback=self.parse_four, meta={'url': q['company_url']})
    ...
    #         print(response.urljoin(company))
    #         companycol.insert_one({'company_url': response.urljoin(company), 'complete': 0})
    ...
    def parse_four(self, response):
        ...
        if response.status == 200:
            try:
                ...
                companycol.update_one({'company_url': response.meta['url']}, {"$set": {'complete': 1}})
                results.insert_one(details)
