...
class QuotesSpider(scrapy.Spider):
    ...
    def spider_closed(self, spider):
        ...
        with open(f"output/partyslate.json", "w", encoding="utf-8") as out:
            out.write(json_data)

    def start_requests(self):
        for i in range(1, 501):    
            yield scrapy.Request(url=f'https://www.partyslate.com/find-vendors?page={i}', callback=self.parse_two)
        
    def parse_two(self, response):

        links = response.css('div.components-FindCompaniesCard-FindCompaniesCard-module__information__1EWlo > div > div.components-FindCompaniesCard-components-Header-Header-module__container__3cZii.components-FindCompaniesCard-FindCompaniesCard-module__header__1ffur > div > h3 > a').extract()
