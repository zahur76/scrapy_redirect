...
mydb = myclient["sourcesecurity"]

mycol = mydb["sourcesecurity"]
...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for i in range(1, 100):
            print(f'page: {i}')
            yield scrapy.Request(url=f'https://www.sourcesecurity.com/companies/security-systems-integrators/directory.html?page={i}', callback=self.parse_two)
            
    def parse_two(self, response):

        links = response.css('div.h3 a::attr("href")').extract()
