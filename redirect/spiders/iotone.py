...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for i in range(1, 109):
            print(f'page: {i}')
            yield scrapy.Request(url=f'https://www.iotone.com/suppliers?page={i}', callback=self.parse_two)
            
    def parse_two(self, response):

        links = response.css('a.product-name::attr("href")').extract()
