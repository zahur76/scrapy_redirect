...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url=f'https://www.cataloxy.co.uk/firms/regions.htm', callback=self.parse_two)
        

    def parse_two(self, response):
        # print(response.url)
        company_links = response.css(".container a.flag::attr('href')").extract()
    ...
    def parse_three(self, response):
        # print(response.url)
        company_links = response.css("ul.cities li a::attr('href')").extract()
        
        print(len(company_links))

        # for company in company_links:
        #     yield scrapy.Request(url=response.urljoin(company), callback=self.parse_four)
